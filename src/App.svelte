<script>
  ...
  function Savenote(){
    const storesPageName = pages[currentPageIndex];
    if(storesPageName != title){
      localStorage.removeItem(storesPageName);
  ...
  function deletePage(index){
    ...
    if(confirm('you want to delete this "${pageTitle}"?')){
...
<main class="p-4 ml-60 h-auto">
  ...
  <hr>
  <textarea class="mt-3 w-full bg-gray-100 border border-gray-300 rounded-lg text-gray-900 p-2.3" bind:value={note}></textarea>
