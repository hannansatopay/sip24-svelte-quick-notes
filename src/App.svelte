      ...
      {#each pages as page, index}
        <li class="flex justify-between items-center">
          <button
            ...
            class="{index === currentPageIndex ? 'bg-dark-gray' : ''} py-2 px-3 text-gray-900 rounded-lg flex-1 text-left"
          >
            {page}
          </button>
          <button
            on:click={() => deletePage(index)}
            class="ml-2 bg-red-500 text-white px-2 py-1 rounded-lg"
          >
            Delete
          </button>
        </li>
...
<main class="p-4 ml-60 h-auto">
  <div class="grid grid-cols-2 items-center mb-3">
    <h1 class="text-3xl font-bold" contenteditable bind:innerHTML={title}></h1>
    <button
