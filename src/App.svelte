  ...
  <div class="bg-light-gray overflow-y-auto py-5 px-3 h-full border-r border-gray-200">
    <ul class="space-y-2">
      ...
      <li class="flex items-center justify-between">
        ...
        <button on:click={() => deletePage(index)} class="{index == currentPageIndex ? 'bg-dark-gray' : ''} py-2 px-3 text-gray-900 rounded-lg flex-grow ml-2">✖</button>
      </li>
      {/each}
...
<main class="p-4 ml-60 h-auto">
  <div class="grid grid-cols-2 items-center mb-3">
    ...
    <div class="flex justify-end">
      ...
      <button on:click={() => deletePage(currentPageIndex)} class="w-20 border border-gray-50 bg-red-600 rounded-lg text-white font-medium hover:bg-red-400 px-4 py-2.5">Delete</button>
    </div>
  </div>
  <hr />
