  ...
  <div class="bg-gray-200 overflow-y-auto py-5 px-3 h-full border-r border-gray-200">
    <ul class="space-y-2">
      {#each pages as page, index}
        <li>
          <button on:click={() => selectPage(index)} class="{index === currentPageIndex ? 'bg-gray-400' : ''} py-2 px-3 text-gray-900 rounded-lg">{page.title}</button>
        </li>
      {/each}
...
<main class="p-4 ml-60 h-auto">
  <div class="flex justify-between items-center mb-3">
    ...
    <div>
      <button class="bg-gray-800 ml-auto text-white px-5 py-2.5 rounded-lg font-medium text-sm mt-3 hover:bg-gray-900" on:click={remove}>Delete</button>
  ...
  <!-- <input type="text" class="block w-full bg-gray-50 border border-gray-300 rounded-lg p-2.5 text-gray-900" name="title" bind:value={title}> -->
