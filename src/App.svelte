    ...
    <ul class="space-y-2">
      {#each pages as page, index}
        <li class="flex items-center">
          ...
          <button on:click={() => deletePage(index)} class="ml-2 text-red-600 hover:text-red-800">&times;</button>
        </li>
      {/each}
...
<main class="p-4 ml-60 h-auto">
  <div class="grid grid-cols-3 items-center mb-3">
    <h1 class="text-3xl font-bold col-span-2" contenteditable bind:textContent={title} on:input={() => { if (title.trim()) saveNote(); }}></h1>
