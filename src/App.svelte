      ...
      <ul class="space-y-2">
        {#each pages as page, index}
          <li class="flex items-center">
            <div class="flex-grow">
              <div contenteditable class="page-button {index === currentPageIndex ? 'current-page' : ''} py-2 px-3 text-gray-900 rounded-lg cursor-pointer"
                on:click={() => selectPage(index)} on:blur={(e) => updatePageTitle(index, e.target.textContent)}
  ...
  <main class="p-4 ml-60 h-auto">
    <div class="grid grid-cols-2 items-center mb-3">
      ...
      <div class="flex justify-end">
        ...
        <button class="bg-blue-500 text-white px-5 py-2.5 rounded-lg font-medium text-sm mt-3 ml-3 hover:bg-blue-900" on:click={saveNote}>Save</button>
      </div>
    </div>
    <hr>
