<script>
  ...
  function saveNote() {
    ...
    if (storedPageName && storedPageName !== title) {
  ...
  <div class="bg-light-gray overflow-y-auto py-5 px-3 h-full border-r border-gray-200">
    <ul class="space-y-2">
      {#each pages as page, index}
        <li>
          <button class="{index === currentPageIndex ? 'bg-dark-gray' : 'bg-light-gray'} py-2 px-3 text-gray-900 rounded-lg" on:click={() => selectPage(index)}>{page}</button>
          <button class="ml-2 text-red-600" on:click={() => deletePage(index)}>Delete</button>
        </li>
      {/each}
